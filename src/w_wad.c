...
wad_file_t *W_AddFile (char *filename)
{
    ...
    if (M_strcasecmp(filename+strlen(filename)-3 , "wad" ) )
    {
	// single lump file
    ...
    filelumps = Z_Malloc(numfilelumps * sizeof(lumpinfo_t), PU_STATIC, 0);
    memset(filelumps, 0, numfilelumps * sizeof(lumpinfo_t));
...
lumpindex_t W_CheckNumForName(char* name)
{
    ...
    if (lumphash != NULL)
    {
        ...
        {
            if (!M_strncasecmp(lumpinfo[i]->name, name, 8))
            {
                return i;
            }
        }
...
void *W_CacheLumpNum(lumpindex_t lumpnum, int tag)
{
	...
	W_ReadLump (lumpnum, lump->cache);
        ...
        // Special case to handle Playdate audio quirk:
        // need to convert sound samples to signed.
        if (tag == PU_SOUND) {
            for (int i = 8; i < lump->size; i++) {
                result[i] ^= 0x80;
            }
        }
    }
