...
char *P_SaveGameFile(int slot)
{
    ...
    if (filename == NULL)
    {
        ...
        filename = Z_Malloc(filename_size, PU_STATIC, 0);
    }
...
boolean P_OpenSaveFile(const char *name, const char *mode) {
    save_stream = fopen(name, mode);
    return save_stream != NULL;
}

void P_CloseSaveFile(void) {
    fclose(save_stream);
}
