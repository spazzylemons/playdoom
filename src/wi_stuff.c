...
{
    V_DrawStretched(40, 0, background);
	V_BlackBars();
}
...
void WI_drawLF(void)
{
    ...
    {
        // draw <LevelName> 
        V_DrawPatchFont((SCREENWIDTH - SHORT(lnames[wbs->last]->width))/2,
                    y, lnames[wbs->last]);

        // draw "Finished!"
        y += (5*SHORT(lnames[wbs->last]->height))/4;

        V_DrawPatchFont((SCREENWIDTH - SHORT(finished->width)) / 2, y, finished);
    }
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
    V_DrawPatchFont((SCREENWIDTH - SHORT(entering->width))/2,
		y,
                entering);
    ...
    V_DrawPatchFont((SCREENWIDTH - SHORT(lnames[wbs->next]->width))/2,
		y, 
                lnames[wbs->next]);
  ...
  patch_t*	c[] )
{
    ...
    if (fits && i<2)
    {
	V_DrawStretched(lnodes[wbs->epsd][n].x,
                    lnodes[wbs->epsd][n].y,
		    c[i]);
    }
    else
...
void WI_drawAnimatedBack(void)
{
	...
	if (a->ctr >= 0)
	    V_DrawStretched(a->loc.x, a->loc.y, a->p[a->ctr]);
    }
  ...
  int		digits )
{
    ...
    {
	x -= fontwidth;
	V_DrawPatchFont(x, y, num[ n % 10 ]);
	n /= 10;
    }

    // draw a minus sign if necessary
    if (neg)
	V_DrawPatchFont(x-=8, y, wiminus);
  ...
  int		t )
{
    ...
    {
	// "sucks"
	V_DrawPatchFont(x - SHORT(sucks->width), y, sucks); 
    }
}
...
void WI_drawStats(void)
{
    ...
    V_DrawPatchFont(SP_STATSX, SP_STATSY, kills);
    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY, cnt_kills[0]);

    V_DrawPatchFont(SP_STATSX, SP_STATSY+lh, items);
    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+lh, cnt_items[0]);

    V_DrawPatchFont(SP_STATSX, SP_STATSY+2*lh, sp_secret);
    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+2*lh, cnt_secret[0]);

    V_DrawPatchFont(SP_TIMEX, SP_TIMEY, timepatch);
    WI_drawTime(SCREENWIDTH/2 - SP_TIMEX, SP_TIMEY, cnt_time);

    if (wbs->epsd < 3)
    {
	V_DrawPatchFont(SCREENWIDTH/2 + SP_TIMEX, SP_TIMEY, par);
