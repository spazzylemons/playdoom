...
void F_CastPrint (char* text)
{
	...
	w = SHORT (hu_font[c]->width);
	V_DrawPatchFont(cx, 180, hu_font[c]);
	cx+=w;
    }
...
void F_BunnyScroll (void)
{
    ...
    scrolled = (240 - ((signed int) finalecount-390)/2);
    if (scrolled > 240)
	scrolled = 240;
    if (scrolled < 0)
	scrolled = 0;
	...
	else if (x+scrolled - p1->width < p2->width)
    ...
    if (finalecount < 1180)
    {
        V_DrawPatchFont((SCREENWIDTH - 13 * 8) / 2,
                        (SCREENHEIGHT - 8 * 8) / 2, 
...
static void F_ArtScreenDrawer(void)
{
    ...
    else
    {
        ...
        V_DrawStretched(40, 0, W_CacheLumpName(lumpname, PU_CACHE));
        V_BlackBars();
    }
}
