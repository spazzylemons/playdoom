...
#define F_W SCREENWIDTH
#define F_H (SCREENHEIGHT - ST_HEIGHT)
...
{
    memset(I_VideoBuffer, 0, SCREENSTRIDE*F_H*sizeof(*I_VideoBuffer));
}
...
static inline void putdot(int x, int y) {
    int xmask = 0x80 >> (x & 7);
    I_VideoBuffer[(y * SCREENSTRIDE) + (x >> 3)] |= xmask;
}
  ...
  int		color )
{
    ...
    // dot counter for drawing full and dotted lines.
    int dotcount = color;
#define PUTDOT(xx, yy) \
    if (dotcount < 2) { \
        putdot(xx, yy); \
...
void AM_drawWalls(void)
{
	...
	if (cheating || (lines[i].flags & ML_MAPPED))
	{
		...
		else if (lines[i].flags & ML_SECRET) // secret door
		{
		    ...
		    AM_drawMline(&l, DOTTED); // floor level change
		}
		else if (lines[i].backsector->ceilingheight
			   != lines[i].frontsector->ceilingheight) {
		    AM_drawMline(&l, DOTTED); // ceiling level change
		}
		else if (cheating) {
		    AM_drawMline(&l, FULL);
		}
	...
	{
	    if (!(lines[i].flags & LINE_NEVERSEE)) AM_drawMline(&l, DOTTED);
	}
    }
}
...
void AM_drawPlayers(void)
{
	...
	if (cheating)
		...
		(cheat_player_arrow, arrlen(cheat_player_arrow), 0,
		 plr->mo->angle, FULL, plr->mo->x, plr->mo->y);
	else
	    AM_drawLineCharacter
		(player_arrow, arrlen(player_arrow), 0, plr->mo->angle,
		 FULL, plr->mo->x, plr->mo->y);
	return;
    }
...
void AM_drawMarks(void)
{
	...
	if (markpoints[i].x != -1)
	{
		...
		V_DrawPatchFont(fx, fy, marknums[i]);
	}
    }
