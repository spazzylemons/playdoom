...
static int FindInList(searchlist_t *list, char *name)
{
    ...
    {
        if (!M_strncasecmp(list->lumps[i]->name, name, 8))
            return i;
    }
...
static sprite_frame_t *FindSpriteFrame(char *name, int frame)
{
    ...
    for (i=0; i<num_sprite_frames; ++i)
    {
        ...
        if (!M_strncasecmp(cur->sprname, name, 4) && cur->frame == frame)
        {
            return cur;
        }
    }
    ...
    // not using strncpy because of gcc warning
...
static void DoMerge(void)
{
    ...
    for (i=0; i<iwad.numlumps; ++i)
    {
        ...
        {
            case SECTION_NORMAL:
                if (!M_strncasecmp(lump->name, "F_START", 8))
                {
                    current_section = SECTION_FLATS;
                }
                else if (!M_strncasecmp(lump->name, "S_START", 8))
                {
            ...
            case SECTION_FLATS:
                ...
                if (!M_strncasecmp(lump->name, "F_END", 8))
                {
            ...
            case SECTION_SPRITES:
                ...
                if (!M_strncasecmp(lump->name, "S_END", 8))
                {
    ...
    for (i=0; i<pwad.numlumps; ++i)
    {
        ...
        switch (current_section)
        {
            case SECTION_NORMAL:
                if (!M_strncasecmp(lump->name, "F_START", 8)
                 || !M_strncasecmp(lump->name, "FF_START", 8))
                {
                    current_section = SECTION_FLATS;
                }
                else if (!M_strncasecmp(lump->name, "S_START", 8)
                      || !M_strncasecmp(lump->name, "SS_START", 8))
                {
                    current_section = SECTION_SPRITES;
                }
                else
            ...
            case SECTION_FLATS:
                ...
                if (!M_strncasecmp(lump->name, "FF_END", 8)
                 || !M_strncasecmp(lump->name, "F_END", 8))
                {
                    // end of section
                    current_section = SECTION_NORMAL;
                }
            ...
            case SECTION_SPRITES:
                ...
                if (!M_strncasecmp(lump->name, "SS_END", 8)
                 || !M_strncasecmp(lump->name, "S_END", 8))
                {
                    // end of section
                    current_section = SECTION_NORMAL;
                }
